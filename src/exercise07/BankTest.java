...
public class BankTest {
    public static void main(String[] args) {
        ...
        Customer customer1 = new Customer("Tony", "Stark"); //고객이름, 계좌잔액 설정
        customer1.setAccount(new BankAccount(100_000)); // 100_000 ??
        ...
        for (int i = 0; i < bank.getNumberOfCustomers(); i++) {
            ...
            if (account.getAccount().getBalance() > max) {
                max = account.getAccount().getBalance(); //최대계좌잔액 반환
                maxBalanceCustomer = account;
            }
            if (account.getAccount().getBalance() < min) {
                min = account.getAccount().getBalance(); //최소계좌잔액 반환
